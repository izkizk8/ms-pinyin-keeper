...
- 监听 `Shift` 键，记录你最近一次实际切换到的中/英模式作为"期望状态"
- 每 250ms 检测当前活动窗口是否变化
- 窗口变化后，如果当前 IME 模式与期望状态不一致，自动模拟一次 `Shift` 把它纠回期望状态
- 通过 `imm32!ImmGetDefaultIMEWnd` + `WM_IME_CONTROL` 读取 IME 真实状态，避免乱按 Shift
